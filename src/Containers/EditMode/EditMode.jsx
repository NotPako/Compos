...
const EditMode = () => {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            ...
            setPartsBlack(list.map(element => <Card className='cardDesign'
            style={{backgroundColor: `${element.color}`, width: `${parseInt(element.length) + 1}rem`}}title={element.name} 
  ...
  return (
    ...
    animate={{opacity: 1}}
    exit={{opacity:0}} className='createDesign'>
        <ElementList partsBlack={partsBlack} setPartsBlack={setPartsBlack} startingParts={whiteList} id={id} title={compo.title}/>
