...
//This useEffect is executed until the blackList hook gets rendered
    useEffect(() => {
        ...
        async function fetchData() {
            ...
            setBlackList(list.map(element => <Card style={{backgroundColor: `${element.color}`, width: `${element.length}`}} title={element.name}/>));
        }
        fetchData();
    }
    }, [blackList]);
...
<div style={{display: 'flex', flexDirection:'column'}}>
    ...
    <Button style={{position:'fixed', bottom:'20px', right:'20px'}}
        onClick={() => navigate('/explore')}>Back to the explorer</Button>
</div>
 )
}
